/**
@file 	FFTcalculator.cpp
@author Lime Microsystems
@brief 	class for performing FFT calculations
*/
        ...
        if(packetsCollected[pkt->channel] >= pthis->m_avgCount)
        {
            if(((pthis->m_channelsEnable >> pkt->channel) & 0x1) == true)
            {
                ...
                for(int s=0; s<fftSize; ++s)
                {
                    ...
                    pkt->amplitudes_dbFS[s] = pkt->amplitudes[s] != 0 ? (20 *log10(pkt->amplitudes[s])) - 69.2369 : val0;
                }
                if(pthis->m_dccorr) // remove dc component from spectrum
                {
                    ...
                    pkt->amplitudes_dbFS[dcIndex] = (pkt->amplitudes_dbFS[dcIndex-1]+pkt->amplitudes_dbFS[dcIndex+1])/2;
                }
                ...
                memset(pkt->amplitudes_dbFS, 0, sizeof(float)*pkt->amplitudesCount);

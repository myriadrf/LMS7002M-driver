...
{
    MakeData();
    unsigned char newbuf[12];
    memset(newbuf, 0, 12);
    memcpy(&newbuf[0], &m_registers[9], 3);
    memcpy(&newbuf[3], &m_registers[6], 3);
    memcpy(&newbuf[6], &m_registers[0], 3);
    memcpy(&newbuf[9], &m_registers[3], 3);
    ...
    if( m_serPort->SendData(CMD_ADF4002_WR, newbuf, 12) == STATUS_COMPLETED_CMD)
        MessageLog::getInstance()->write("ADF4002 configured.\n");
    else
        MessageLog::getInstance()->write("ADF4002 configuration failed.\n");
}

void ADF_module::SetFrefFvco(double Fref, double Fvco, int &rcount, int &ncount)
{
    txtFref = Fref;
    txtFvco = Fvco;
    CalculateRN();

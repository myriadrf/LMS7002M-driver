        ...
        else // CLK6 and CLK7 only integer mode
        {
            if(CLK[i].outputFreqHz <= 150000000)
            {
                if(i==6)
                {
                    ...
                    if(DivA%2 != 0)
                    {
                        MessageLog::getInstance()->write("Si5351C - CLK6 multisynth divider is not even integer\n", LOG_ERROR);
                        return false;
                    }
                }
                else
                {
                    ...
                    if(DivA%2 != 0)
                    {
                        MessageLog::getInstance()->write("Si5351C - CLK7 multisynth divider is not even integer\n", LOG_ERROR);
                        return false;
                    }
                }
            }
...
Si5351C::Status Si5351C::GetStatus()
{
    unsigned char out[2];
    unsigned char in[2];
    long unsigned inLen = 2;
    out[0] = 0;
    out[1] = 1;
    if(STATUS_COMPLETED_CMD == device->SendReadData(CMD_SI5351_RD, out, 2, in, inLen))
    {
        Status stat;
        stat.sys_init = (in[0] >> 7);
        stat.lol_b = (in[0] >> 6) & 0x1;
        stat.lol_a = (in[0] >> 5) & 0x1;
        stat.los = (in[0] >> 4) & 0x1;
        stat.sys_init_stky = (in[1] >> 7);
        stat.lol_b_stky = (in[1] >> 6) & 0x1;
        stat.lol_a_stky = (in[1] >> 5) & 0x1;
        stat.los_stky = (in[1] >> 4) & 0x1;
        return stat;
    }
    else
    {
...
void Si5351C::ClearStatus()
{
    unsigned char out[2];
    out[0] = 1;
    out[1] = 0x0;
    device->SendData(CMD_SI5351_WR, out, 2);
}

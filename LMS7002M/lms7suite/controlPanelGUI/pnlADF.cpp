...
const long pnlADF::ID_BUTTON2 = wxNewId();
//*)
...
void pnlADF::Initialize(ADF_module* pModule)
{
    m_pModule = pModule;
}
...
void pnlADF::BuildContent(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
    ...
    Connect(ID_BUTTON2,wxEVT_COMMAND_BUTTON_CLICKED,(wxObjectEventFunction)&pnlADF::OnbtnUploadClick);
    //*)
}
...
void pnlADF::OnbtnCalcSendClick(wxCommandEvent& event)
{
    ...
    m_pModule->SendConfig();
}

void pnlADF::OnbtnUploadClick(wxCommandEvent& event)
{
    //reference counter latch
    int ldp = cmbLDP->GetSelection();
    int abw = cmbABW->GetSelection();
